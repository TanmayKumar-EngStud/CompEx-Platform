...
class BasePromptGenerator(ABC):
    ...
    def __init__(
        ...
        self.logger = StructuredLogger(
            f"PromptGenerator_{exam_type.value}_{section_type.value}")
    ...
    def _load_customizations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            customizations_path = Path(
                "system_instructions") / exam_name / "customizations.json"
    ...
    def get_difficulty_pool(self) -> List[int]:
        ...
        """
        difficulty_ratio = self.difficulty_distribution.get(
            str(self.mock_difficulty), {})
        ...
        medium_count = int(total_questions *
                           difficulty_ratio.get("medium", 0.6))
    ...
    def generate_nomenclature_based_prompt(
        ...
        nomenclature = question_config.get(
            "nomenclature", "") if question_config else ""
    ...
    def _substitute_nomenclature_variables(
        ...
        for var in variables:
            ...
            if "vocabulary_level:" in var:
                ...
                substituted = substituted.replace(
                    f"<{var}>", f"<vocabulary_level: {vocab_level}>")
                continue
            ...
            if "|" in var:
                ...
                for individual_var in individual_vars:
                    ...
                    if any(char.isdigit() for char in clean_individual_var):
                        import re
                        base_var_match = re.match(
                            r'^([a-zA-Z_]+?)_?\d', clean_individual_var)
                        if base_var_match:
                            ...
                            if base_var in ["question_style", "focused_skill"]:
                                child_config = question_config.get(
                                    "child-question", {})
                                individual_values = child_config.get(
                                    base_var, [])
                                # Fallback to main config if not found in child-question
                                if not individual_values:
                                    individual_values = question_config.get(
                                        base_var, [])
                            else:
                                individual_values = question_config.get(
                                    base_var, [])
            ...
            if var == "focused_skill":
                ...
                if hasattr(self, '_topic_skills') and self._topic_skills:
                    selected_skill = self.get_random_element(
                        self._topic_skills)
                    substituted = substituted.replace(
            ...
            filtered_values = self._filter_values_by_type(
                var_values, var_clean, selected_type)
    ...
    def _filter_values_by_type(
        ...
        if selected_type:
            ...
            matching_values = [
                v for v in values if v and selected_type in str(v)]
            if matching_values:
            ...
            for v in values:
                if not v:
                    continue
                has_selector = any(selector in str(
                    v) for selector in selectors if selector != selected_type)
                if not has_selector:
                    clean_values.append(str(v).replace(
                        selected_type, '') if selected_type in str(v) else str(v))
            return clean_values
