...
class TestQuestionComponentTemplates(unittest.TestCase):
    ...
    def test_component_creation_with_template_loading(self):
        ...
        test_cases = [
            ...
            (QuestionType.READING_COMPREHENSION, ExamType.GMAT, ParentChildQuestion),
            (QuestionType.READING_COMPREHENSION, ExamType.GRE, ParentChildQuestion),
            (QuestionType.CRITICAL_REASONING, ExamType.GMAT, SimpleQuestion),
            (QuestionType.GRAPHIC_INTERPRETATION, ExamType.GMAT, GraphicInterpretationQuestion),
            ...
            (QuestionType.MULTI_SOURCE_REASONING, ExamType.GMAT, MultiSourceReasoningQuestion),
        ]
                ...
                try:
                    ...
                    if hasattr(component, '_get_component_instruction'):
                        for component_name in ["QuestionText", "QuestionSolution"]:
                            try:
                                instruction = component._get_component_instruction(component_name)
                                self.assertIsNotNone(instruction)
                                self.assertIsInstance(instruction, str)
                            except Exception as e:
                                self.fail(
    ...
    def test_data_sufficiency_template_mapping(self):
        ...
        for exam_type in [ExamType.GMAT, ExamType.GRE]:
            try:
                ...
                self.fail(f"Data sufficiency template loading failed for {exam_type.value}: {e}")
    ...
    def test_problem_solving_template_mapping(self):
        ...
        for exam_type in [ExamType.GMAT, ExamType.GRE]:
            try:
                ...
                self.fail(f"Problem solving template loading failed for {exam_type.value}: {e}")
    ...
    def test_template_content_validity(self):
        ...
        test_cases = [
            (QuestionType.DATA_SUFFICIENCY, "questionText", ["passage", "statements", "question"]),
    ...
    def test_missing_templates_detection(self):
        ...
        standard_modes = ["questionText", "questionTitle", "questionSolution", "questionAnswer"]
            ...
            for exam_type in [ExamType.GMAT, ExamType.GRE]:
                ...
                for mode in standard_modes:
                    try:
                        ...
                        missing_templates.append(f"{exam_type.value}/{question_type.value}/{mode}: {e}")
