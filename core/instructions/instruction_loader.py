...
class InstructionLoader:
    ...
    def _validate_new_system(self) -> None:
        ...
        if not system_instructions_path.exists():
            raise FileNotFoundError(
                "system_instructions directory not found - unified instruction system required")

        templates_path = system_instructions_path / "templates"
        if not templates_path.exists():
            raise FileNotFoundError(
                "templates directory not found - unified instruction system required")
        ...
        if not gmat_customizations.exists():
            raise FileNotFoundError(
                "GMAT customizations file not found - unified instruction system required")
    ...
    def _load_template_file(self, question_type: QuestionType, mode: str) -> str:
        ...
        component_map = {
            ...
            "questionOptions": "4-questionOptions",
            "dichotomousChoiceOptions": "4-questionOptions",
            "questionSolution": "3-questionSolution",
        ...
        question_style_filename = question_style_map.get(
            question_style, "0-generic")

        template_path = Path("system_instructions") / "templates" / \
            component_folder / f"{question_style_filename}.txt.template"

        if not template_path.exists():
            # Fall back to generic template in the same component folder
            template_path = Path("system_instructions") / "templates" / \
                component_folder / "0-generic.txt.template"

        if not template_path.exists():
            raise TemplateLoadError(
                f"Template not found for mode '{mode}' and question_type '{question_type.value}'")
    ...
    def _load_customization_file(
        ...
        customization_path = Path(
            "system_instructions") / exam_type.value / "customizations.json"
        ...
        try:
            ...
            question_customizations = customizations.get(
                question_type.value, {})
            self._logger.log_customization_loaded(
                str(customization_path), len(question_customizations))
    ...
    def _load_main_instruction_file(self, question_type: QuestionType) -> str:
        ...
        template_path = Path("system_instructions") / "templates" / \
            "!Main Instructions" / template_filename

        if not template_path.exists():
            # Fall back to generic template
            template_path = Path("system_instructions") / "templates" / \
                "!Main Instructions" / "0_generic.txt.template"

        if not template_path.exists():
            raise TemplateLoadError(
                f"Main instruction template not found for question_type '{question_type.value}'")
        ...
        try:
            ...
            raise TemplateLoadError(
                f"Failed to load main instruction template {template_path}: {e}")
