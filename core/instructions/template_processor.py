...
E: "Statements (1) and (2) TOGETHER are NOT sufficient."'''
    ...
    def process_template(
        ...
        try:
            ...
            variables = self._prepare_variables(
                exam_type, question_type, customizations)
            ...
            if prompt:
                processed = self._graph_style_manager.inject_graph_style(
                    processed, prompt)

            # Apply content format token replacement
            processed = self._process_content_format_tokens(processed, prompt)
            ...
            self._logger.log_template_processing_error(
                e, exam_type, question_type)
        ...
        for pattern in inheritance_patterns:
            if pattern in template:
                try:
                    ...
                    generic_template_path = Path(
                        "system_instructions") / "templates" / "!Main Instructions" / "0_generic.txt.template"
    ...
    def _process_content_format_tokens(self, template: str, prompt: Optional[str] = None) -> str:
        """Process content format tokens like {passage_format}, {table_format}, etc."""
        try:
            # Load metadata.json for content formats
            metadata_path = Path("system_instructions") / \
                "templates" / "0-questionMetadata" / "metadata.json"
            ...
            # Replace {passage_format} token
            if "{passage_format}" in template:
                passage_format = self._get_passage_format(metadata, prompt)
                template = template.replace("{passage_format}", passage_format)

            # Replace {table_format} token
            if "{table_format}" in template:
                table_format = self._get_table_format(metadata, prompt)
                template = template.replace("{table_format}", table_format)

            # Replace {graph_format} token (for future use)
            if "{graph_format}" in template:
                graph_format = self._get_graph_format(metadata, prompt)
                template = template.replace("{graph_format}", graph_format)

            # Replace {content} token with appropriate format based on content type in prompt
            if "{content}" in template:
                content_format = self._get_content_format_from_prompt(
                    metadata, prompt)
                template = template.replace("{content}", content_format)

            return template

        except Exception as e:
            # If processing fails, return template as-is
            self._logger.log_template_processing_error(e, None, None)
            return template

    def _get_passage_format(self, metadata: Dict[str, Any], prompt: Optional[str] = None) -> str:
        """Get appropriate passage format from metadata."""
        try:
            passage_styles = metadata.get("content_types", {}).get(
                "passage", {}).get("passage_styles", {})

            # Default to reading_comprehension format
            default_style = "reading_comprehension"

            # Determine style from prompt if available
            style = default_style
            if prompt:
                prompt_lower = prompt.lower()
                if "critical" in prompt_lower or "reasoning" in prompt_lower:
                    style = "critical_reasoning"
                elif "reading" in prompt_lower or "comprehension" in prompt_lower:
                    style = "reading_comprehension"

            format_data = passage_styles.get(
                style, passage_styles.get(default_style, {}))
            json_format = format_data.get("json_format", {})

            # Return formatted JSON example
  ...
  "type": "passage",
  "title": "Passage Title",
  "content": "Main passage text content here..."
}'''

    def _get_table_format(self, metadata: Dict[str, Any], prompt: Optional[str] = None) -> str:
        """Get appropriate table format from metadata."""
        try:
            table_styles = metadata.get("content_types", {}).get(
                "table", {}).get("table_styles", {})

            # Default to data_table format
            default_style = "data_table"

            # Determine style from prompt if available
            style = default_style
            if prompt:
                prompt_lower = prompt.lower()
                if "pivot" in prompt_lower:
                    style = "pivot_table"
                elif "comparison" in prompt_lower or "compare" in prompt_lower:
                    style = "comparison_table"
                elif "data" in prompt_lower or "table" in prompt_lower:
                    style = "data_table"

            format_data = table_styles.get(
                style, table_styles.get(default_style, {}))
            json_format = format_data.get("json_format", {})
  ...
  "data": [
    ...
    def _get_graph_format(self, metadata: Dict[str, Any], prompt: Optional[str] = None) -> str:
        """Get appropriate graph format from metadata."""
        try:
            chart_styles = metadata.get("content_types", {}).get(
                "graph", {}).get("chart_styles", {})

            # Default to bar_chart format
            default_style = "bar_chart"

            # Determine style from prompt if available
            style = default_style
            if prompt:
                prompt_lower = prompt.lower()
                graph_mapping = metadata.get("content_types", {}).get(
                    "graph", {}).get("graph_type_mapping", {})
                for key, mapped_style in graph_mapping.items():
                    if key in prompt_lower:
                        style = mapped_style
                        break

            format_data = chart_styles.get(
                style, chart_styles.get(default_style, {}))
            json_format = format_data.get("json_format", {})
  ...
  "y_axis": "Y Axis Label",
  "data": [
    {"category": "Category 1", "value": 25, "color": "#3498db"},
    {"category": "Category 2", "value": 45, "color": "#e74c3c"}
  ]
}'''

    def _get_content_format_from_prompt(self, metadata: Dict[str, Any], prompt: Optional[str] = None) -> str:
        """Get appropriate content format based on prompt content type."""
        if not prompt:
            return "content_data_here"

        try:
            prompt_lower = prompt.lower()

            # Check for graph/chart types
            graph_mapping = metadata.get("content_types", {}).get(
                "graph", {}).get("graph_type_mapping", {})
            for key, mapped_style in graph_mapping.items():
                if key in prompt_lower:
                    return self._get_graph_format(metadata, prompt)

            # Check for table types
            table_mapping = metadata.get("content_types", {}).get(
                "table", {}).get("table_type_mapping", {})
            for key, mapped_style in table_mapping.items():
                if key in prompt_lower:
                    return self._get_table_format(metadata, prompt)

            # Check for passage types
            passage_mapping = metadata.get("content_types", {}).get(
                "passage", {}).get("passage_type_mapping", {})
            for key, mapped_style in passage_mapping.items():
                if key in prompt_lower:
                    return self._get_passage_format(metadata, prompt)

            # Check for general content type mapping
            content_mapping = metadata.get("content_type_mapping", {})
    ...
    def _get_question_specific_variables(self, question_type: QuestionType) -> Dict[str, str]:
        ...
        if question_type in [QuestionType.GRAPHIC_INTERPRETATION, QuestionType.TABLE_ANALYSIS]:
            variables[
                "VISUAL_ELEMENT_REQUIREMENT"] = "Generate appropriate visual elements (graphs, tables) that support the question"
