...
def determine_target_exam(papers_dir, exam_definition):
    """
    Determines which exam to generate based on file counts.
    Rule: If GRE count <= GMAT count -> Generate GRE. Else -> GMAT.
    """
    counts = {}
    for exam in ['GRE', 'GMAT']:
        exam_dir = os.path.join(papers_dir, exam)
        os.makedirs(exam_dir, exist_ok=True)
        # Count only valid json files
        files = [f for f in os.listdir(exam_dir) if f.startswith(exam) and f.endswith('.json')]
        counts[exam] = len(files)
    
    print(f"\n{prettify('CURRENT FILE COUNTS:', 'Cyan')}")
    print(f"GRE: {counts['GRE']} | GMAT: {counts['GMAT']}")
    
    if counts['GRE'] <= counts['GMAT']:
        return 'GRE'
    else:
        return 'GMAT'
...
def process_exam_generation():
    ...
    # 2. Determine Target Exam
    target_exam = determine_target_exam(papers_dir, exam_definition)
        ...
        for q_type in section_data['question types']:
            ...
            for _ in range(count):
                ...
                current_question_data = {
                    ...
                    'difficulty': diff,
                    'question-type': q_type,
                    'tags': parse_tags(prompt_str, q_type)
                }
