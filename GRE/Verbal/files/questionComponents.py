    ...
    def generate_passages(self):
        def _generate(warn=False):
            ...
            if "rc-s" in self.prompt.lower():
                ...
                print(
                    f"GRE_Verbal: this is self.prompt: {self.prompt} thus total_passage_count didn't worked")
                return None
        # endregion
    ...
    def generate_parentTitle(self):
        def _generate(warn=False):
            ...
            try:
                ...
                print(
                    f"Failed json parsing for question title here is how the response looked like : {response}")
                print(f"error: {str(e)}")
        result = self._retry_generate(_generate)
    ...
    def generate_childQuestionTitle(self, index):
        def _generate(warn=False):
            ...
            try:
                ...
                print(
                    f"GRE Verbal, error in generate_childQuestionTitle: {str(e)}")
                return None
        result = self._retry_generate(_generate)
    ...
    def generate_childOptions(self, index):
        def _generate(warn=False):
            ...
            try:
                message = json.loads(refine_response(response))
                options = message.get(
                    "options", f"error generating childOptions for {self.prompt}")
                answer = message.get(
                    "answer", f"error generating childOptions for {self.prompt}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if result:
                    return result
                print(
                    f"🚩 {func_name} has failed to return response - attempt {attempt+1}/{max_retries}")
                print(f"\tReason: Function returned None")
            except Exception as e:
                print(
                    f"🏳️ {func_name} failed in even attempting [SimpleQuestion-GRE Verbal]because: {str(e)}")
        return None
    ...
    def generate_questionText(self):
        def _generate(warn=False):
            ...
            try:
                ...
                res = message.get(
                    'question', f'question keyword is not getting captured {self.prompt}')
                return res
            except Exception as e:
    ...
    def generate_questionSolution(self):
        def _generate(warn=False):
            ...
            try:
                ...
                print(f"GRE Verbal, error in generate_childSolution: {str(e)}")
                print(
                    f"this is the response that is being captured: {response}")
            return None
