...
-   `3-questionOptions`: Templates for generating the answer options.
-   `4-questionSolution`: Templates for generating the detailed solution.
...
3.  **InstructionLoader**: The `InstructionManager` then delegates the task of finding and loading the correct template to the `InstructionLoader`. The loader uses a two-step mapping process:
    *   **`component_map`**: This dictionary maps the requested `mode` (e.g., `questionOptions`) to a specific folder within the `templates` directory (e.g., `3-questionOptions`).
...
3.  **Loader**: 
    -   `component_map` maps `'questionOptions'` to the `3-questionOptions` folder.
    -   `question_style_map` maps `'text_completion'` to the `3-text_completion.txt.template` file.
4.  **Loading**: The loader reads `system_instructions/templates/3-questionOptions/3-text_completion.txt.template` and the `text_completion` section from `system_instructions/gre/customizations.json`.
